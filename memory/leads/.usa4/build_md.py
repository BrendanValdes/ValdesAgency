...
md = f"""# POOL USA 4 — National Pool Service Dial Sheet
**Built:** 2026-07-14 · **Leads:** {len(rows)} · **Confidence:** 100% live-verified (Google knowledge-panel rating/reviews/phone).
**Dedup:** 0 collisions vs the rebuilt book (744 names / 738 phones, incl. POOL USA 1/2/3 + LV/PHX/UT/PEST/GARAGE) · 0 within-batch dups · every lead has a phone.
...
**Method:** Wide SERP harvest across an expanded pool-belt city map (USA3 metros retained for recall + new LA/OC, inland Bay, secondary TX/FL/GA/SC/NC + warm-belt fill) × 4 query variants (service / cleaning / maintenance / repair) → snack_pack + organic candidates → name-dedup vs the full 744-name book → per-business Google knowledge-panel enrichment (verified rating/reviews/phone/site/address) → 3-signal tiered gap filter on VERIFIED data → phone-dedup. Zero reasoning agents per lead — pure parallel search + enrich. Owner defaults to business name where unknown (never blank, never guessed).

**Next move:** Dial Tier 1 top-down — Reputation/Website leads book fastest. Roll into Tier 2 once Tier 1 is worked.
"""
open(f"{D}/../POOL USA 4.md", "w").write(md)
print("wrote POOL USA 4.md — leads:", len(rows), "| Tier1:", len(t1), "Tier2:", len(t2), "| metros:", len(metros))
