...
// public by default:
//   ig-container  — renders a card, serves it on PUBLIC_BASE_URL, creates an
//                   IG media container (validates public fetch + JPEG + auth),
//                   does NOT publish. Containers expire in 24h on their own.
...
// The health server must already be serving /i/ on that base URL.
// ============================================================================
...
import { publicImageUrl, renderCardToVolume } from "../src/services/image-cards.js";
import { initStateStore, getState, mutateState } from "../src/services/state.js";
...
switch (mode) {
  case "ig-container": {
    ...
    const { getState: gs } = await import("../src/services/state.js");
    let igUserId = gs().igUserId;
    if (!igUserId) {
  ...
  case "fb-unpublished": {
    ...
    const { getBrand: gb } = await import("../src/services/brand-config.js");
    const b = await gb("valdes");
    const pages = await executeTool("FACEBOOK_GET_USER_PAGES", fbConn, {});
    console.log("pages:", JSON.stringify(pages).slice(0, 400));
