...
class BudgetTracker {
  ...
  constructor(allowed: OfflineRunBudget) {
...
function validateInput(
  ...
  if (input.executionScope !== "offline_synthetic") {
    throw new Error("Offline orchestration requires the offline_synthetic execution scope");
  }
  if (
    ...
    throw new Error("Offline orchestration requires networking, paid providers, and external verification to be disabled");
  }
  if (
    ...
    throw new Error("Offline orchestration requires pool_service as the sole enabled/default niche");
  }
  if (input.nicheId !== "pool_service" || dependencies.niche.id !== input.nicheId) {
    throw new Error(`Unsupported or disabled niche: ${input.nicheId}`);
  }
  if (!dependencies.niche.enabled || !dependencies.policy.enabledNiches.includes(dependencies.niche.id)) {
    throw new Error(`Niche ${input.nicheId} is disabled`);
  }
  const providerPolicy = requireProviderPolicy(dependencies.policy, input.providerId);
  if (
    ...
    throw new Error(`Provider ${input.providerId} is not permitted for synthetic offline discovery`);
  }
  const provider = dependencies.providerRegistry.require(input.providerId);
  if (provider.providerId !== input.providerId) {
    throw new Error("Provider registry returned a mismatched provider");
  }
  if (dependencies.fixtureFetcher.sourceClass !== "synthetic_fixture") {
    throw new Error("Website assessment requires an injected synthetic fixture fetcher");
  }
  if (!input.runKey.trim()) throw new Error("Offline run key is required");
  if (!input.fixtureWebsite.fixtureId.trim()) throw new Error("Fixture identity is required");
  if (!input.fixtureWebsite.providerResultId.trim()) {
    throw new Error("Fixture provider result identity is required");
  }
  if (!input.fixtureWebsite.expectedBusinessName.trim()) {
    throw new Error("Fixture business name is required");
  }
  if (input.market.length === 0) throw new Error("At least one coverage target is required");
  if (!input.queryVersion.trim()) throw new Error("Query version is required");
  if (input.orchestrationVersion !== OFFLINE_ORCHESTRATION_VERSION) {
    throw new Error("Unsupported offline orchestration version");
  }
  if (input.extractionVersion !== EXTRACTION_POLICY_VERSION) {
    throw new Error("Offline extraction version does not match the executable extractor policy");
  }
  validateBudget(input.budget);
  const fixtureUrl = validatedFixtureUrl(input);
  if (!dependencies.fixtureFetcher.handles(fixtureUrl)) {
    throw new Error("Fixture website URL has no explicit synthetic fetcher mapping");
  }
  return fixtureUrl;
}
...
export async function runOfflineLeadAssessment(
  ...
  const existing = dependencies.database.prepare(`
    SELECT run_id, run_key, input_hash, status, result_json
    FROM offline_orchestration_runs WHERE run_key = ?
  `).get(input.runKey) as OfflineRunRow | undefined;
  if (existing) {
    if (existing.input_hash !== inputHash) {
      throw new Error("Offline run key is already bound to different deterministic input");
    }
    if (existing.result_json) {
      return JSON.parse(existing.result_json) as OfflineLeadPipelineResult;
    }
    throw new Error(`Offline run ${existing.run_id} is ${existing.status}; durable recovery is outside Phase 3D1`);
  }
  ...
  withTransaction(dependencies.database, () => {
    ...
    `).run(
      ...
      JSON.stringify(budget.snapshot().consumed),
      startedAt,
      startedAt,
    );
  ...
  const finalize = (
    ...
    const offlineStatus = terminalStatus === "review_required" ? "review_required" : terminalStatus;
    ...
    withTransaction(dependencies.database, () => {
      dependencies.database.prepare(`
        ...
        SET status = ?, usage_json = ?, review_required = ?, assessment_attachment = ?,
            denial_reason = ?, result_json = ?, completed_at = ?, updated_at = ?
        WHERE run_id = ?
      `).run(
        offlineStatus,
  ...
  try {
    ...
    state.coverage = dependencies.coveragePlanner({
      nicheId: dependencies.niche.id,
    ...
    state.queries = state.coverage.cells.flatMap((cell) =>
    ...
    withTransaction(dependencies.database, () => {
      ...
      dependencies.database.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        state.coverage?.manifestId,
        configurationId,
        input.queryVersion,
        state.coverage?.strategy,
        state.coverage?.resultCap,
        state.coverage?.maxDepth,
        state.coverage?.minimumSpan,
        dependencies.clock.now(),
      );
    ...
    const provider = dependencies.providerRegistry.require(input.providerId);
    const envelopes: Array<ProviderEnvelope<NormalizedDiscoveryResult>> = [];
    const queryByCorrelation = new Map<string, DiscoveryQuery>();
    const providerCallByCorrelation = new Map<string, string>();
    for (const query of state.queries) {
      ...
      queryByCorrelation.set(correlationId, query);
      providerCallByCorrelation.set(correlationId, providerCallId);
      ...
      if (batch.envelopes.some((envelope) =>
        ...
        throw new Error("Synthetic discovery provider returned non-offline provenance or cost");
      }
    ...
    state.observations = dependencies.resultNormalizer(envelopes);
    withTransaction(dependencies.database, () => {
      for (const observation of state.observations) {
        const query = queryByCorrelation.get(observation.correlationId);
        const providerCallId = providerCallByCorrelation.get(observation.correlationId);
      ...
      for (const envelope of envelopes.filter((value) => value.validation.status === "rejected")) {
        const query = queryByCorrelation.get(envelope.correlationId);
        const providerCallId = providerCallByCorrelation.get(envelope.correlationId);
        ...
        `).run(
          ...
          providerCallId,
          envelope.providerId,
          envelope.providerSchemaVersion,
          envelope.correlationId,
          envelope.providerResultId,
          envelope.observedAt,
          envelope.retrievedAt,
          envelope.error?.category ?? "schema_validation_failed",
          envelope.cache.status,
          envelope.rawReferenceChecksum,
    ...
    state.selectedObservation = state.observations.find((observation) =>
      observation.providerResultId === input.fixtureWebsite.providerResultId
    ) ?? null;
    if (!state.selectedObservation) {
      throw new Error("Synthetic fixture discovery did not produce the requested website identity");
    }
    ...
    const candidate = observationIdentity(state.selectedObservation, dependencies);
    const existingIdentities = dependencies.existingIdentities(state.selectedObservation);
    ...
    if (!dependencies.fixtureFetcher.handles(fixtureUrl)) {
      throw new Error("Fixture website mapping disappeared before assessment");
    }
    ...
    for (const page of successfulPages) {
      ...
      allServices.push(...dependencies.extractors.extractServiceEvidence({
        ...
        jsonLd: page.jsonLd,
        niche: dependencies.niche,
        providerCategories: state.selectedObservation.result.categories,
        providerSourceClass: state.selectedObservation.sourceClass,
      }));
    ...
    withTransaction(dependencies.database, () => {
      ...
      for (const crawlPage of crawl.pages) {
        ...
        const extracted = successfulPages.find((page) => page.crawlPage === crawlPage);
    ...
    const result = finalize(terminalStatus, []);
    ...
    if (error instanceof PipelineCancelled) {
      ...
      const result = finalize("cancelled", [`cancelled_during_${error.stage}:${reason}`]);
      event(error.stage, "cancelled", { reason });
      return result;
    }
    if (error instanceof PipelineBudgetBlocked) {
      state.warnings.push(error.reason);
      const result = finalize("budget_blocked", [error.reason]);
    ...
    try {
      withTransaction(dependencies.database, () => {
        dependencies.database.prepare(`
          UPDATE offline_orchestration_runs
          SET status = 'failed', usage_json = ?, denial_reason = ?, completed_at = ?, updated_at = ?
          WHERE run_id = ?
        `).run(
          JSON.stringify(budget.snapshot().consumed),
          error instanceof Error ? error.message : "unknown_pipeline_failure",
          failedAt,
          failedAt,
          runId,
        );
        dependencies.database.prepare(`
          UPDATE lead_runs SET state = 'failed', spent_micro_usd = 0, updated_at = ? WHERE id = ?
        `).run(failedAt, runId);
        dependencies.database.prepare(`
          UPDATE run_stages SET state = 'failed', finished_at = ?, updated_at = ? WHERE id = ?
        `).run(failedAt, failedAt, stageId);
        dependencies.database.prepare(`
          UPDATE stage_tasks
          SET state = CASE WHEN state IN ('accepted', 'human_review') THEN state ELSE 'failed' END,
              reason_code = CASE WHEN state IN ('accepted', 'human_review') THEN reason_code ELSE 'provider_failed' END,
              updated_at = ?
          WHERE stage_id = ?
        `).run(failedAt, stageId);
      });
      event("finalization", "failed", {
        reason: error instanceof Error ? error.message : "unknown_pipeline_failure",
      });
    } catch {
