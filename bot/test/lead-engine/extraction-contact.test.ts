...
describe("public contact extraction", () => {
  it("retains link and structured-data provenance without verification upgrades", () => {
    ...
    expect(contacts.every(({ sourceClass, claimState }) => sourceClass === "public_business_website" && claimState === "public_unverified_candidate")).toBe(true);
    expect(JSON.stringify(contacts)).not.toContain("externally_verified");
