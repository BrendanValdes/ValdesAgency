...
describe("robots policy", () => {
  ...
  it("uses wildcard fallback with longest-match allow semantics", async () => {
    ...
    try {
      const service = new RobotsPolicyService({ fetcher: createSyntheticLoopbackFetcher({ allowedOrigin: server.origin }) });
  ...
  it("allows a missing robots file but fails closed on robots fetch failure", async () => {
    ...
    try {
      const service = new RobotsPolicyService({ fetcher: createSyntheticLoopbackFetcher({ allowedOrigin: missing.origin }) });
    ...
    try {
      const service = new RobotsPolicyService({ fetcher: createSyntheticLoopbackFetcher({ allowedOrigin: failed.origin }) });
  ...
  it("expires cached robots decisions at the explicit TTL", async () => {
    ...
    try {
      const service = new RobotsPolicyService({
        fetcher: createSyntheticLoopbackFetcher({ allowedOrigin: server.origin }),
        ttlMs: 1_000,
        now: () => now,
      });
