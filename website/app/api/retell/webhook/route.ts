...
async function crmRequest(
  ...
  headers.set(
    "Authorization",
    `Bearer ${requireEnvironmentVariable("GHL_PRIVATE_INTEGRATION_TOKEN")}`,
  );
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (process.env.AVA_EXTERNAL_WRITES_ENABLED !== "true") {
    console.error("[retell-webhook] Ava external writes are disabled");
    return NextResponse.json(
      { ok: false, error: "Ava CRM writes are currently disabled" },
      { status: 503 },
    );
  }
